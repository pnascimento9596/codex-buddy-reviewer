...
- **Receipt replay validation (H07).** Every replay path now runs the context-free structural receipt validation (schema version, review-key binding, canonical creation timestamp, exact terminal key set, terminal/result coherence) before presenting a continuation. Structurally invalid or malformed legacy pairs — including the demonstrated forged-pair class (bad schema, bad timestamp, forged finding content) — are rejected and the completion record is marked `invalid_pre_review_receipt`. A valid legacy receipt still replays correctly.
  - **Scope note (threat model):** this closes the malformed/forged-structure class. A shape-conformant pair written entirely by a process that already has write access to both private-state files remains indistinguishable from legitimate state at replay — same-user malicious-process defense is outside this product's accepted threat model (see docs/SECURITY.md).
