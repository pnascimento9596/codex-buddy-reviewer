<div align="center">
  ...
  <p>
    <a href="#status"><img alt="Version v0.5.0-rc.3" src="https://img.shields.io/badge/version-v0.5.0--rc.3-7c3aed" /></a>
...
Release candidate: `v0.5.0-rc.3`, prepared for publication only from the exact protected `main` head through the guarded release workflow.
...
After protected publication completes, install the published `v0.5.0-rc.3` release through Codex's Git marketplace support:

```bash
codex plugin marketplace add pnascimento9596/codex-buddy-reviewer --ref v0.5.0-rc.3
codex plugin add codex-buddy-reviewer@codex-buddy-reviewer --json
codex plugin list
```

`v0.5.0-rc.3` includes the clean-filter and filtered-path privacy fixes from PRs #17 and #19, release-owner authorization from PR #21, the reproduced Phase 2 cleanup/publication fixes from PR #22, and the adversarial corpus and checked-JavaScript gates from PRs #23 and #24. Install the immutable tag, not the moving `main` branch.

This repository includes a Claude Code marketplace manifest under `.claude-plugin/` for packaging layout compatibility. Claude Code is **not** a supported Buddy host in `v0.5.0-rc.3`: hooks and skills currently implement the Codex plugin lifecycle only. Use the Codex CLI marketplace install path above.
...
The two-command marketplace flow was verified locally with Codex CLI `0.144.4` against this repository layout. The public `v0.5.0-rc.3` tag is created only after the protected release workflow rebuilds, verifies, attests, and publishes the artifact-only release candidate. The stable `v0.5.0` tag remains unavailable until the security, platform, and five-pet Codex host gates close. During private development, use the existing `personal` marketplace flow.
