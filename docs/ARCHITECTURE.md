...
The source registry exposes four adapter IDs for compatibility: `claude`, `grok`, `ollama`, and `opencode`. The current Lane J operator-supported review choices are Claude, Ollama, and explicitly configured OpenCode connections; Grok is unavailable for this lane and is not used in its review evidence. A mode contains one ordered primary descriptor and at most one ordered secondary descriptor. The same exact evidence request is prepared for both lanes except for the optional primary-only summary packet. Adapter selection is explicit and never falls back.
...
- The legacy `grok` adapter remains source-compatible but is unavailable for the current operator lane and is not used in Lane J review evidence.
...
- `opencode` invokes a pure, disposable, deny-all OpenCode agent. The model must use `provider/model` form. Buddy projects only that selected provider entry from the OpenCode auth store into the disposable environment, disables external skills, project configuration, MCP, sharing, tools, and ambient provider credentials, and then validates the returned JSON locally. `--pure` keeps external and project plugins out. OpenCode default first-party provider plugins are permitted because OpenCode 1.18+ resolves provider/model IDs through them; Buddy does not set `OPENCODE_DISABLE_DEFAULT_PLUGINS`, and the isolation environment allowlist is unchanged. Supported routed examples include ChatGPT Plus/Pro OAuth, API-backed Kimi or Moonshot, and Ollama Cloud when each exact provider/model is already configured in OpenCode. These are routed connections, not extra native Buddy adapters. Claude Pro or Max is intentionally excluded from this route and must use Buddy's direct Claude Code adapter.
